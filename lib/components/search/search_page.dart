...
class SearchPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final users = List<AppUser>.from(AppUser.values)
...
class __UserProfileState extends State<_UserProfile> {
  ...
  late Future<MyUser> userDataFuture = getUser();
  ...
  Future<MyUser> getUser() async {
    ...
    return MyUser.fromMap(streamUser.data!);
  }
    ...
    return Row(
      children: [
        Padding(
          ...
          child: Avatar.medium(myUser: widget.userData),
        ),
        Padding(
          padding: const EdgeInsets.all(8.0),
          child: Column(
